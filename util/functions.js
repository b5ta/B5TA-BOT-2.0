  ...
  */
  client.permlevel = message => {
    ...
    while (permOrder.length) {
      ...
      if (message.server && currentLevel.serverOnly) continue;
  ...
  */
  client.clean = async (client, text) => {
    ...
    text = text
      ...
      .replace(/@/g, "@" + String.fromCharCode(8203))
      .replace(client.token, client.config.token);
  ...
  client.loadBoss = (bossName) => {
    try {
      ...
      return `Unable to load boss ${bossName}: ${e}`;
    }
  };

  client.unloadBoss = async (bossName) => {
    let boss;

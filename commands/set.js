...
// This command is to modify/edit server configuration. Perm Level 3 for admins
...
exports.run = async (client, message, [action, key, ...value], level) => { // eslint-disable-line no-unused-vars
  ...
  const settings = client.settings.get(message.server.id);
  ...
  if (action === "add") {
    ...
    client.settings.set(message.server.id, settings);
